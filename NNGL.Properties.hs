...
instance Arbitrary Dimension where
  arbitrary = elements (map Dimension [1..100])
...
prop_hline (Dimension w) = isCorrectPicture line && width line == w && height line == 1
  where line = hline w

prop_vline :: Dimension -> Bool
prop_vline (Dimension h) = isCorrectPicture line && height line == h && width line == 1
  where line = vline h

prop_rect :: Dimension -> Dimension -> Bool
prop_rect (Dimension w) (Dimension h) = isCorrectPicture r && width r == w && height r == h
  where r = rect w h
...
-- above
prop_above :: Pict -> Pict -> PictureCombinator -> Bool
prop_above p1 p2 comb = prop_comb_dim p1 p2 comb height width

prop_aboveEdge :: Pict -> Pict -> PictureCombinator -> Int -> Bool
prop_aboveEdge p1@(Pict pict1) p2@(Pict pict2) comb c = prop_above p1 p2 comb &&
                                                        if pict1W /= pict2W then isBlankRect newPict (r, c) w h else True
  where newPict = pict1 `comb` pict2
        r = if pict1W < pict2W then 0 else height pict1
        w = abs (pict1W - pict2W)
...
prop_aboveCenter p1@(Pict pict1) p2@(Pict pict2) = prop_above p1 p2 aboveCenter &&
                                                   if pict1W /= pict2W then isBlankRect newPict (r, cl) wl h &&
                                                                            isBlankRect newPict (r, cr) wr h
                                                                       else True
  where newPict = pict1 `aboveCenter` pict2
        r = if pict1W < pict2W then 0 else height pict1
        cl = 0
        wl = floor wBlanks
        h = height $ if pict1W < pict2W then pict1 else pict2
...
-- below
prop_below p1 p2 = prop_above p2 p1
